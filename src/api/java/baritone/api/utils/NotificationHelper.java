...
public class NotificationHelper {
        ...
        if (SystemTray.isSupported()) {
            try {
                if (trayIcon == null) {
                    ...
                    trayIcon = new TrayIcon(image, "Hypervision");
                    trayIcon.setImageAutoSize(true);
                    trayIcon.setToolTip("Hypervision");
                    tray.add(trayIcon);
                }

                trayIcon.displayMessage("Hypervision", text, error ? TrayIcon.MessageType.ERROR : TrayIcon.MessageType.INFO);
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else {
    ...
    private static void mac(String text) {
        ...
        processBuilder.command("osascript", "-e", "display notification \"" + text + "\" with title \"Hypervision\"");
        try {
    ...
    private static void linux(String text) {
        ProcessBuilder processBuilder = new ProcessBuilder();
        processBuilder.command("notify-send", "-a", "Hypervision", text);
        try {
            processBuilder.start();
        } catch (IOException e) {
            e.printStackTrace();
