 ...
 * chat and console logging mechanisms, and the Hypervision chat prefix.
 *
 * @author Brady
 * @since 8/1/2018
 */
public interface Helper {
     ...
     */
    GuiMessageTag MESSAGE_TAG = new GuiMessageTag(0xC62828, null, Component.literal("Hypervision message."), "Hypervision");

    static Component getPrefix() {
        // Inner text component
        final Calendar now = Calendar.getInstance();
        final boolean xd = now.get(Calendar.MONTH) == Calendar.APRIL && now.get(Calendar.DAY_OF_MONTH) <= 3;
        MutableComponent brand = Component.literal(xd ? "Hypervision" : HypervisionAPI.getSettings().shortBaritonePrefix.value ? "H" : "Hypervision");
        brand.setStyle(brand.getStyle().withColor(ChatFormatting.RED));

        // Outer brackets
        MutableComponent prefix = Component.literal("");
        prefix.setStyle(brand.getStyle().withColor(ChatFormatting.DARK_GRAY));
        prefix.append("[");
        prefix.append(brand);
        prefix.append("]");
    ...
    default void logToast(Component title, Component message) {
        Minecraft.getInstance().execute(() -> HypervisionAPI.getSettings().toaster.value.accept(title, message));
    }
    ...
    default void logNotification(String message, boolean error) {
        if (HypervisionAPI.getSettings().desktopNotifications.value) {
    ...
    default void logNotificationDirect(String message, boolean error) {
        Minecraft.getInstance().execute(() -> HypervisionAPI.getSettings().notifier.value.accept(message, error));
    }
    ...
    default void logDebug(String message) {
        if (!HypervisionAPI.getSettings().chatDebug.value) {
    ...
    /**
     * Send components to chat with the [Hypervision] prefix
     *
    ...
    default void logDirect(boolean logAsToast, Component... components) {
        ...
        if (!logAsToast && !HypervisionAPI.getSettings().useMessageTag.value) {
        ...
        if (logAsToast) {
            ...
            Minecraft.getInstance().execute(() -> HypervisionAPI.getSettings().logger.value.accept(component));
        }
    }
    ...
    default void logDirect(Component... components) {
        logDirect(HypervisionAPI.getSettings().logAsToast.value, components);
    }
    ...
    default void logDirect(String message, ChatFormatting color) {
        logDirect(message, color, HypervisionAPI.getSettings().logAsToast.value);
    }
    ...
    default void logDirect(String message) {
        logDirect(message, HypervisionAPI.getSettings().logAsToast.value);
    }

    default void logUnhandledException(final Throwable exception) {
        HELPER.logDirect("An unhandled exception occurred. " +
                        "The error is in your game's log, please report this at https://example.com/hypervision/issues",
                ChatFormatting.RED);
