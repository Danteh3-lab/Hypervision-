...
public final class ElytraBehavior implements Helper {
    private static final Color HYPERVISION_CRIMSON = new Color(0xC62828);
    private static final Color HYPERVISION_SLATE = new Color(0x4B5563);
    private static final Color HYPERVISION_LIGHT = new Color(0xD1D5DB);
    ...
    public void onRenderPass(RenderEvent event) {
        ...
        if (this.visiblePath != null) {
            PathRenderer.drawPath(event.getModelViewStack(), this.visiblePath, 0, HYPERVISION_CRIMSON, false, 0, 0, 0.0D);
        }
        if (this.aimPos != null) {
            PathRenderer.drawGoal(event.getModelViewStack(), ctx, new GoalBlock(this.aimPos), event.getPartialTicks(), HYPERVISION_LIGHT);
        }
        if (!this.clearLines.isEmpty() && settings.elytraRenderRaytraces.value) {
            BufferBuilder bufferBuilder = IRenderer.startLines(HYPERVISION_LIGHT, settings.pathRenderLineWidthPixels.value);
        ...
        if (!this.blockedLines.isEmpty() && Baritone.settings().elytraRenderRaytraces.value) {
            BufferBuilder bufferBuilder = IRenderer.startLines(HYPERVISION_SLATE, settings.pathRenderLineWidthPixels.value);
