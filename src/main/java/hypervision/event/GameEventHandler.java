...
public final class GameEventHandler implements IEventBus, Helper {
    ...
    public GameEventHandler(Hypervision hypervision) {
    ...
    public final void onTick(TickEvent event) {
        if (event.getType() == TickEvent.Type.IN) {
            try {
                hypervision.bsi = new BlockStateInterface(hypervision.getPlayerContext(), true);
            } catch (Exception ex) {
                ex.printStackTrace();
                hypervision.bsi = null;
            }
        } else {
            hypervision.bsi = null;
        }
    ...
    public void onChunkEvent(ChunkEvent event) {
        ...
        Level world = hypervision.getPlayerContext().world();
        ...
        if (event.isPostPopulate() || isPreUnload) {
            hypervision.getWorldProvider().ifWorldLoaded(worldData -> {
    ...
    public void onBlockChange(BlockChangeEvent event) {
        if (hypervision.settings().repackOnAnyBlockChange.value) {
            ...
            if (keepingTrackOf) {
                hypervision.getWorldProvider().ifWorldLoaded(worldData -> {
                    final Level world = hypervision.getPlayerContext().world();
    ...
    public final void onWorldEvent(WorldEvent event) {
        WorldProvider cache = hypervision.getWorldProvider();
