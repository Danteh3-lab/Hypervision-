...
public class GuiClick extends Screen implements Helper {
    ...
    public void render(PoseStack stack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (near != null && far != null) {
            ...
            LocalPlayer player = HypervisionAPI.getProvider().getPrimaryBaritone().getPlayerContext().player();
        ...
        if (currentMouseOver != null) { //Catch this, or else a click into void will result in a crash
            if (mouseButton == 0) {
                if (clickStart != null && !clickStart.equals(currentMouseOver)) {
                    HypervisionAPI.getProvider().getPrimaryBaritone().getSelectionManager().removeAllSelections();
                    HypervisionAPI.getProvider().getPrimaryBaritone().getSelectionManager().addSelection(BetterBlockPos.from(clickStart), BetterBlockPos.from(currentMouseOver));
                    MutableComponent component = Component.literal("Selection made! For usage: " + hypervision.settings().prefix.value + "help sel");
                    ...
                    HypervisionAPI.getProvider().getPrimaryBaritone().getCustomGoalProcess().setGoalAndPath(new GoalBlock(currentMouseOver));
                }
            } else if (mouseButton == 1) {
                HypervisionAPI.getProvider().getPrimaryBaritone().getCustomGoalProcess().setGoalAndPath(new GoalBlock(currentMouseOver.above()));
            }
        }
        clickStart = null;
    ...
    public void onRender(PoseStack modelViewStack, Matrix4f projectionMatrix) {
        ...
        if (currentMouseOver != null) {
            ...
            PathRenderer.drawManySelectionBoxes(modelViewStack, e, Collections.singletonList(currentMouseOver), new Color(0x9CA3AF));
            if (clickStart != null && !clickStart.equals(currentMouseOver)) {
                IRenderer.startLines(new Color(0xC62828), hypervision.settings().pathRenderLineWidthPixels.value, true);
