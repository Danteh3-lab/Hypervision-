...
public class ToolSet {
    ...
    public ToolSet(LocalPlayer player) {
        ...
        if (hypervision.settings().considerPotionEffects.value) {
    ...
    public int getBestSlot(Block b, boolean preferSilkTouch, boolean pathingCalculation) {
        ...
        */
        if (!hypervision.settings().autoTool.value && pathingCalculation) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (!hypervision.settings().useSwordToMine.value && itemStack.getItem() instanceof SwordItem) {
                continue;
            }

            if (hypervision.settings().itemSaver.value && (itemStack.getDamageValue() + hypervision.settings().itemSaverThreshold.value) >= itemStack.getMaxDamage() && itemStack.getMaxDamage() > 1) {
                continue;
            }
    ...
    private double avoidanceMultiplier(Block b) {
        return hypervision.settings().blocksToAvoidBreaking.value.contains(b) ? hypervision.settings().avoidBreakingMultiplier.value : 1;
    }
