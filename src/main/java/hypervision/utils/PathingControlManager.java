...
import hypervision.api.pathing.calc.IPathingControlManager;
...
public class PathingControlManager implements IPathingControlManager {
    ...
    public PathingControlManager(Hypervision hypervision) {
        this.hypervision = hypervision;
        this.processes = new HashSet<>();
        this.active = new ArrayList<>();
        hypervision.getGameEventHandler().registerEventListener(new AbstractGameEventListener() { // needs to be after all behavior ticks
            @Override
    ...
    public void preTick() {
        ...
        inControlThisTick = null;
        PathingBehavior p = hypervision.getPathingBehavior();
    ...
    private void postTick() {
        ...
        switch (command.commandType) {
            case FORCE_REVALIDATE_GOAL_AND_PATH:
                ...
                break;
            case REVALIDATE_GOAL_AND_PATH:
                if (hypervision.settings().cancelOnGoalInvalidation.value && (command.goal == null || revalidateGoal(command.goal))) {
                    p.softCancelIfSafe();
                }
                p.secretInternalSetGoalAndPath(command);
                break;
    ...
    public boolean forceRevalidate(Goal newGoal) {
        PathExecutor current = hypervision.getPathingBehavior().getCurrent();
        if (current != null) {
    ...
    public PathingCommand executeProcesses() {
        ...
        while (iterator.hasNext()) {
            ...
            PathingCommand exec = proc.onTick(Objects.equals(proc, inControlLastTick) && hypervision.getPathingBehavior().calcFailedLastTick(), hypervision.getPathingBehavior().isSafeToCancel());
            if (exec == null) {
                if (proc.isActive()) {
