...
import hypervision.api.behavior.IPathingBehavior;
...
import hypervision.api.utils.PathCalculationResult;
import hypervision.api.utils.interfaces.IGoalRenderPos;
import hypervision.pathing.calc.AStarPathFinder;
...
import hypervision.pathing.path.PathExecutor;
...
import hypervision.utils.pathing.Favoring;
...
public final class PathingBehavior extends Behavior implements IPathingBehavior, Helper {
    ...
    public PathingBehavior(Hypervision hypervision) {
        super(hypervision);
    }
    ...
    private void dispatchEvents() {
        ...
        for (PathEvent event : curr) {
            hypervision.getGameEventHandler().onPathEvent(event);
        }
    }
    ...
    public void onTick(TickEvent event) {
        ...
        if (event.getType() == TickEvent.Type.OUT) {
            ...
            hypervision.getPathingControlManager().cancelEverything();
            return;
        }

        expectedSegmentStart = pathStart();
        hypervision.getPathingControlManager().preTick();
        tickPath();
        ticksElapsedSoFar++;
        dispatchEvents();
    }
    ...
    private void tickPath() {
        ...
        if (pauseRequestedLastTick && safeToCancel) {
            ...
            if (unpausedLastTick) {
                ...
                hypervision.getInputOverrideHandler().getBlockBreakHelper().stopBreakingBlock();
            }
        ...
        synchronized (pathPlanLock) {
            ...
            if (current.failed() || current.finished()) {
                ...
                if (goal == null || goal.isInGoal(ctx.playerFeet())) {
                    ...
                    next = null;
                    if (hypervision.settings().disconnectOnArrival.value) {
            ...
            if (hypervision.settings().splicePath.value) {
            ...
            synchronized (pathCalcLock) {
                ...
                if (ticksRemainingInSegment(false).get() < hypervision.settings().planningTickLookahead.value) {
    ...
    public boolean secretInternalSetGoalAndPath(PathingCommand command) {
        ...
        if (command instanceof PathingCommandContext) {
            ...
            context = new CalculationContext(hypervision, true);
        }
        if (goal == null) {
            return false;
        }
    ...
    public boolean isSafeToCancel() {
        if (current == null) {
            return !hypervision.getElytraProcess().isActive() || hypervision.getElytraProcess().isSafeToCancel();
        }
        return safeToCancel;
    }
    ...
    public boolean cancelEverything() {
        ...
        hypervision.getPathingControlManager().cancelEverything(); // regardless of if we can stop the current segment, we can still stop the processes
        return doIt;
    }
    ...
    private void findPathInNewThread(final BlockPos start, final boolean talkAboutIt, CalculationContext context) {
        ...
        long failureTimeout;
        if (current == null) {
            primaryTimeout = hypervision.settings().primaryTimeoutMS.value;
            failureTimeout = hypervision.settings().failureTimeoutMS.value;
        } else {
            primaryTimeout = hypervision.settings().planAheadPrimaryTimeoutMS.value;
            failureTimeout = hypervision.settings().planAheadFailureTimeoutMS.value;
        }
    ...
    private AbstractNodeCostSearch createPathfinder(BlockPos start, Goal goal, IPath previous, CalculationContext context) {
        ...
        if (hypervision.settings().simplifyUnloadedYCoord.value && goal instanceof IGoalRenderPos) {
