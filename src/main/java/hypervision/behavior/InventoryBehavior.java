...
public final class InventoryBehavior extends Behavior implements Helper {
    ...
    public InventoryBehavior(Hypervision hypervision) {
        super(hypervision);
    }
    ...
    private boolean requestSwapWithHotBar(int inInventory, int inHotbar) {
        ...
        if (ticksSinceLastInventoryMove < hypervision.settings().ticksBetweenInventoryMoves.value) {
            logDebug("Inventory move requested but delaying " + ticksSinceLastInventoryMove + " " + hypervision.settings().ticksBetweenInventoryMoves.value);
            return false;
        }
        if (hypervision.settings().inventoryMoveOnlyIfStationary.value && !hypervision.getInventoryPauserProcess().stationaryForInventoryMove()) {
    ...
    private int firstValidThrowaway() { // TODO offhand idk
        ...
        for (int i = 0; i < invy.size(); i++) {
            if (hypervision.settings().acceptableThrowawayItems.value.contains(invy.get(i).getItem())) {
                return i;
            }
        }
        return -1;
    ...
    private int bestToolAgainst(Block against, Class<? extends DiggerItem> cla$$) {
        ...
        for (int i = 0; i < invy.size(); i++) {
            ...
            if (hypervision.settings().itemSaver.value && (stack.getDamageValue() + hypervision.settings().itemSaverThreshold.value) >= stack.getMaxDamage() && stack.getMaxDamage() > 1) {
                continue;
            }
    ...
    public boolean hasGenericThrowaway() {
        for (Item item : hypervision.settings().acceptableThrowawayItems.value) {
    ...
    public boolean selectThrowawayForLocation(boolean select, int x, int y, int z) {
        BlockState maybe = hypervision.getBuilderProcess().placeAt(x, y, z, hypervision.bsi.get0(x, y, z));
    ...
    public boolean throwaway(boolean select, Predicate<? super ItemStack> desired) {
        return throwaway(select, desired, hypervision.settings().allowInventory.value);
    }
