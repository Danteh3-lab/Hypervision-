...
import hypervision.api.command.datatypes.RelativeFile;
...
import static hypervision.api.utils.SettingsUtil.*;

public class SetCommand extends Command {

    public SetCommand(IBaritone hypervision) {
        super(hypervision, "set", "setting", "settings");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (Arrays.asList("s", "save").contains(arg)) {
            SettingsUtil.save(hypervision.settings());
        ...
        if (Arrays.asList("load", "ld").contains(arg)) {
            ...
            // reset to defaults
            SettingsUtil.modifiedSettings(hypervision.settings()).forEach(Settings.Setting::reset);
            // then load from disk
            SettingsUtil.readAndApply(hypervision.settings(), file);
        ...
        if (paginate) {
            ...
            List<? extends Settings.Setting> toPaginate =
                    (viewModified ? SettingsUtil.modifiedSettings(hypervision.settings()) : hypervision.settings().allSettings).stream()
            ...
            Paginator.paginate(
                    ...
                    setting -> {
                        ...
                        String commandSuggestion = hypervision.settings().prefix.value + String.format("set %s ", setting.getName());
        ...
        Settings.Setting<?> setting = hypervision.settings().allSettings.stream()
        ...
        if (!doingSomething && !args.hasAny()) {
            ...
            if (resetting) {
                ...
                try {
                    SettingsUtil.parseAndApply(hypervision.settings(), arg, newValue);
                } catch (Throwable t) {
                    t.printStackTrace();
            ...
            if ((setting.getName().equals("chatControl") && !(Boolean) setting.value && !hypervision.settings().chatControlAnyway.value) ||
                    setting.getName().equals("chatControlAnyway") && !(Boolean) setting.value && !hypervision.settings().chatControl.value) {
        ...
        if (args.hasAny()) {
            ...
            if (args.hasExactlyOne() && !Arrays.asList("s", "save").contains(args.peekString().toLowerCase(Locale.US))) {
                if (arg.equalsIgnoreCase("reset")) {
                    ...
                    return RelativeFile.tabComplete(args, SettingsUtil.settingsDirectory().toFile());
                }
                Settings.Setting setting = hypervision.settings().byLowerName.get(arg.toLowerCase(Locale.US));
                if (setting != null) {
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "Using the set command, you can manage all of hypervision's settings. Almost every aspect is controlled by these settings - go wild!",
                "",
                "Usage:",
                ...
                "> set load [filename] - Load settings from another file in your minecraft/hypervision"
        );
    }
}
