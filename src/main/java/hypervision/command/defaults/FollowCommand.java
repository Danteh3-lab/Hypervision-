...
import hypervision.api.command.datatypes.EntityClassById;
import hypervision.api.command.datatypes.IDatatypeFor;
import hypervision.api.command.datatypes.NearbyPlayer;
import hypervision.api.command.exception.CommandErrorMessageException;
...
public class FollowCommand extends Command {
    ...
    public FollowCommand(IBaritone hypervision) {
        super(hypervision, "follow");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (args.hasExactlyOne()) {
            hypervision.getFollowProcess().follow((group = args.getEnum(FollowGroup.class)).filter);
        } else {
            args.requireMin(2);
            group = null;
            ...
            hypervision.getFollowProcess().follow(
                    classes.isEmpty()
                            ? entities::contains
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The follow command tells hypervision to follow certain kinds of entities.",
                "",
                "Usage:",
