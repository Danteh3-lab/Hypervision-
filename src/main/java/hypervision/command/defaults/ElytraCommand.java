...
import hypervision.api.process.ICustomGoalProcess;
import hypervision.api.process.IElytraProcess;
...
public class ElytraCommand extends Command {
    ...
    public ElytraCommand(IBaritone hypervision) {
        super(hypervision, "elytra");
    }

    @Override
    public void execute(String label, IArgConsumer args) throws CommandException {
        final ICustomGoalProcess customGoalProcess = hypervision.getCustomGoalProcess();
        final IElytraProcess elytra = hypervision.getElytraProcess();
        ...
        if (!args.hasAny()) {
            if (hypervision.settings().elytraTermsAccepted.value) {
                if (detectOn2b2t()) {
                    warn2b2t();
                }
            } else {
    ...
    private void warn2b2t() {
        if (hypervision.settings().elytraPredictTerrain.value) {
            long seed = hypervision.settings().elytraNetherSeed.value;
    ...
    private Component suggest2b2tSeeds() {
        ...
        clippy.append("Within a few hundred blocks of spawn/axis/highways/etc, the terrain is too fragmented to be predictable. hypervision Elytra will still work, just with backtracking. ");
        ...
        olderSeed.setStyle(olderSeed.getStyle().withUnderlined(true).withBold(true).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal(hypervision.settings().prefix.value + "set elytraNetherSeed " + OLD_2B2T_SEED))).withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, FORCE_COMMAND_PREFIX + "set elytraNetherSeed " + OLD_2B2T_SEED)));
        ...
        newerSeed.setStyle(newerSeed.getStyle().withUnderlined(true).withBold(true).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal(hypervision.settings().prefix.value + "set elytraNetherSeed " + NEW_2B2T_SEED))).withClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, FORCE_COMMAND_PREFIX + "set elytraNetherSeed " + NEW_2B2T_SEED)));
    ...
    private void gatekeep() {
        ...
        gatekeep.append("hypervision Elytra is an experimental feature. It is only intended for long distance travel in the Nether using fireworks for vanilla boost. It will not work with any other mods (\"hacks\") for non-vanilla boost. ");
        MutableComponent gatekeep2 = Component.literal("If you want hypervision to attempt to take off from the ground for you, you can enable the elytraAutoJump setting (not advisable on laggy servers!). ");
        gatekeep2.setStyle(gatekeep2.getStyle().withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal(hypervision.settings().prefix.value + "set elytraAutoJump true"))));
        gatekeep.append(gatekeep2);
        MutableComponent gatekeep3 = Component.literal("If you want hypervision to go slower, enable the elytraConserveFireworks setting and/or decrease the elytraFireworkSpeed setting. ");
        gatekeep3.setStyle(gatekeep3.getStyle().withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.literal(hypervision.settings().prefix.value + "set elytraConserveFireworks true\n" + hypervision.settings().prefix.value + "set elytraFireworkSpeed 0.6\n(the 0.6 number is just an example, tweak to your liking)"))));
        gatekeep.append(gatekeep3);
        MutableComponent gatekeep4 = Component.literal("hypervision Elytra ");
        ...
        if (detectOn2b2t()) {
            ...
            if (!hypervision.settings().elytraPredictTerrain.value) {
                gatekeep5.append(hypervision.settings().prefix.value + "elytraPredictTerrain is currently disabled. ");
            } else {
                if (hypervision.settings().elytraNetherSeed.value == NEW_2B2T_SEED) {
                    gatekeep5.append("You are using the newer seed. ");
                } else if (hypervision.settings().elytraNetherSeed.value == OLD_2B2T_SEED) {
                    gatekeep5.append("You are using the older seed. ");
                } else {
                    gatekeep5.append("Defaulting to the newer seed. ");
                    hypervision.settings().elytraNetherSeed.value = NEW_2B2T_SEED;
                }
            }
            gatekeep.append(gatekeep5);
        } else {
            if (hypervision.settings().elytraNetherSeed.value == NEW_2B2T_SEED) {
                MutableComponent gatekeep5 = Component.literal("hypervision doesn't know the seed of your world. Set it with: " + hypervision.settings().prefix.value + "set elytraNetherSeed seedgoeshere\n");
                gatekeep5.append("For the time being, elytraPredictTerrain is defaulting to false since the seed is unknown.");
                gatekeep.append(gatekeep5);
                hypervision.settings().elytraPredictTerrain.value = false;
            } else {
                if (hypervision.settings().elytraPredictTerrain.value) {
                    MutableComponent gatekeep5 = Component.literal("hypervision Elytra is predicting terrain assuming that " + hypervision.settings().elytraNetherSeed.value + " is the correct seed. Change that with " + hypervision.settings().prefix.value + "set elytraNetherSeed seedgoeshere, or disable it with " + hypervision.settings().prefix.value + "set elytraPredictTerrain false");
                    gatekeep.append(gatekeep5);
                } else {
                    MutableComponent gatekeep5 = Component.literal("hypervision Elytra is not predicting terrain. If you don't know the seed, this is the correct thing to do. If you do know the seed, input it with " + hypervision.settings().prefix.value + "set elytraNetherSeed seedgoeshere, and then enable it with " + hypervision.settings().prefix.value + "set elytraPredictTerrain true");
                    gatekeep.append(gatekeep5);
                }
            }
        }
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The elytra command tells hypervision to, in the nether, automatically fly to the current goal.",
                "",
                "Usage:",
                ...
                "> elytra supported - Tells you if hypervision ships a native library that is compatible with your PC."
        );
    }
