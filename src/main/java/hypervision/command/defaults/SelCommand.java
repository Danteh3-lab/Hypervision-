...
import hypervision.api.command.datatypes.ForAxis;
import hypervision.api.command.datatypes.ForBlockOptionalMeta;
import hypervision.api.command.datatypes.ForDirection;
import hypervision.api.command.datatypes.RelativeBlockPos;
...
import hypervision.api.command.exception.CommandInvalidTypeException;
import hypervision.api.command.helpers.TabCompleteHelper;
import hypervision.api.event.events.RenderEvent;
import hypervision.api.event.listener.AbstractGameEventListener;
import hypervision.api.schematic.*;
import hypervision.api.schematic.mask.shape.CylinderMask;
import hypervision.api.schematic.mask.shape.SphereMask;
import hypervision.api.selection.ISelection;
import hypervision.api.selection.ISelectionManager;
import hypervision.api.utils.BetterBlockPos;
import hypervision.api.utils.BlockOptionalMeta;
import hypervision.api.utils.BlockOptionalMetaLookup;
import hypervision.utils.BlockStateInterface;
import hypervision.utils.IRenderer;
import hypervision.utils.schematic.StaticSchematic;
...
public class SelCommand extends Command {
    ...
    private ISelectionManager manager = hypervision.getSelectionManager();
    private BetterBlockPos pos1 = null;
    private ISchematic clipboard = null;
    private Vec3i clipboardOffset = null;

    public SelCommand(IBaritone hypervision) {
        super(hypervision, "sel", "selection", "s");
        hypervision.getGameEventHandler().registerEventListener(new AbstractGameEventListener() {
            @Override
            public void onRenderPass(RenderEvent event) {
                if (!hypervision.settings().renderSelectionCorners.value || pos1 == null) {
                    return;
                }
                Color color = hypervision.settings().colorSelectionPos1.value;
                float opacity = hypervision.settings().selectionOpacity.value;
                float lineWidth = hypervision.settings().selectionLineWidth.value;
                boolean ignoreDepth = hypervision.settings().renderSelectionIgnoreDepth.value;
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (action == Action.POS1 || action == Action.POS2) {
            ...
            hypervision.getBuilderProcess().build("Fill", composite, origin);
            ...
            hypervision.getBuilderProcess().build("Fill", clipboard, pos.offset(clipboardOffset));
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The sel command allows you to manipulate hypervision's selections, similarly to WorldEdit.",
                "",
