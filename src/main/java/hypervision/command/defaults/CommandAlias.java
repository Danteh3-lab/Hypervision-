...
public class CommandAlias extends Command {
    ...
    public CommandAlias(IBaritone hypervision, List<String> names, String shortDesc, String target) {
        super(hypervision, names.toArray(new String[0]));
        this.shortDesc = shortDesc;
        this.target = target;
    }

    public CommandAlias(IBaritone hypervision, String name, String shortDesc, String target) {
        super(hypervision, name);
        this.shortDesc = shortDesc;
        this.target = target;
    }
    ...
    public void execute(String label, IArgConsumer args) {
        this.hypervision.getCommandManager().execute(String.format("%s %s", target, args.rawRest()));
    }

    @Override
    public Stream<String> tabComplete(String label, IArgConsumer args) {
        return this.hypervision.getCommandManager().tabComplete(String.format("%s %s", target, args.rawRest()));
    }
