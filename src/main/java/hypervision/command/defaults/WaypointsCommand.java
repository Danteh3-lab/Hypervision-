...
import hypervision.api.command.datatypes.ForWaypoints;
...
public class WaypointsCommand extends Command {
    ...
    public WaypointsCommand(IBaritone hypervision) {
        super(hypervision, "waypoints", "waypoint", "wp");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (action == Action.LIST) {
            ...
            IWaypoint[] waypoints = tag != null
                    ? ForWaypoints.getWaypointsByTag(this.hypervision, tag)
                    : ForWaypoints.getWaypoints(this.hypervision);
            ...
            ForWaypoints.waypoints(this.hypervision).addWaypoint(waypoint);
            ...
            IWaypoint[] waypoints = ForWaypoints.getWaypointsByTag(this.hypervision, tag);
            for (IWaypoint waypoint : waypoints) {
                ForWaypoints.waypoints(this.hypervision).removeWaypoint(waypoint);
            }
            deletedWaypoints.computeIfAbsent(hypervision.getWorldProvider().getCurrentWorld(), k -> new ArrayList<>()).addAll(Arrays.<IWaypoint>asList(waypoints));
            ...
            List<IWaypoint> deletedWaypoints = this.deletedWaypoints.getOrDefault(hypervision.getWorldProvider().getCurrentWorld(), Collections.emptyList());
            ...
            waypoints.forEach(ForWaypoints.waypoints(this.hypervision)::addWaypoint);
            ...
            if (waypoint == null) {
                ...
                if (action == Action.INFO) {
                    ...
                    recreateComponent.setStyle(recreateComponent.getStyle().withClickEvent(new ClickEvent(
                            ...
                            String.format(
                                    "%s%s save %s %s %s %s %s",
                                    hypervision.settings().prefix.value, // This uses the normal prefix because it is run by the user.
                                    label,
                                    waypoint.getTag().getName(),
                                    waypoint.getName(),
                    ...
                    ForWaypoints.waypoints(this.hypervision).removeWaypoint(waypoint);
                    deletedWaypoints.computeIfAbsent(hypervision.getWorldProvider().getCurrentWorld(), k -> new ArrayList<>()).add(waypoint);
                    ...
                    Goal goal = new GoalBlock(waypoint.getLocation());
                    hypervision.getCustomGoalProcess().setGoalAndPath(goal);
    ...
    public List<String> getLongDesc() {
        return Arrays.asList(
                "The waypoint command allows you to manage hypervision's waypoints.",
                "",
