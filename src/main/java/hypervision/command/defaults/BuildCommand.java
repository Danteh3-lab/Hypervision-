...
public class BuildCommand extends Command {
    ...
    public BuildCommand(IBaritone hypervision) {
        super(hypervision, "build");
        this.schematicsDir = new File(hypervision.getPlayerContext().minecraft().gameDirectory, "schematics");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (FilenameUtils.getExtension(file.getAbsolutePath()).isEmpty()) {
            file = new File(file.getAbsolutePath() + "." + hypervision.settings().schematicFallbackExtension.value);
        }
        if (!file.exists()) {
            if (file0.exists()) {
        ...
        boolean success = hypervision.getBuilderProcess().build(file.getName(), file, buildOrigin);
        if (!success) {
