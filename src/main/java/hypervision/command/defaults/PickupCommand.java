...
import hypervision.api.command.datatypes.ItemById;
...
public class PickupCommand extends Command {
    ...
    public PickupCommand(IBaritone hypervision) {
        super(hypervision, "pickup");
    }
    ...
    public void execute(String label, IArgConsumer args) throws CommandException {
        ...
        if (collecting.isEmpty()) {
            hypervision.getFollowProcess().pickup(stack -> true);
            logDirect("Picking up all items");
        } else {
            hypervision.getFollowProcess().pickup(stack -> collecting.contains(stack.getItem()));
