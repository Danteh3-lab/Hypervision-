...
package hypervision.pathing.calc;
...
import hypervision.pathing.calc.openset.BinaryHeapOpenSet;
import hypervision.pathing.movement.CalculationContext;
import hypervision.pathing.movement.Moves;
import hypervision.utils.pathing.BetterWorldBorder;
import hypervision.utils.pathing.Favoring;
import hypervision.utils.pathing.MutableMoveResult;
...
public final class AStarPathFinder extends AbstractNodeCostSearch {
    ...
    protected Optional<IPath> calculate0(long primaryTimeout, long failureTimeout) {
        ...
        boolean slowPath = hypervision.settings().slowPath.value;
        if (slowPath) {
            logDebug("slowPath is on, path timeout will be " + hypervision.settings().slowPathTimeoutMS.value + "ms instead of " + primaryTimeout + "ms");
        }
        long primaryTimeoutTime = startTime + (slowPath ? hypervision.settings().slowPathTimeoutMS.value : primaryTimeout);
        long failureTimeoutTime = startTime + (slowPath ? hypervision.settings().slowPathTimeoutMS.value : failureTimeout);
        ...
        int pathingMaxChunkBorderFetch = hypervision.settings().pathingMaxChunkBorderFetch.value; // grab all settings beforehand so that changing settings during pathing doesn't cause a crash or unpredictable behavior
        double minimumImprovement = hypervision.settings().minimumImprovementRepropagation.value ? MIN_IMPROVEMENT : 0;
        ...
        while (!openSet.isEmpty() && numEmptyChunk < pathingMaxChunkBorderFetch && !cancelRequested) {
            ...
            if (slowPath) {
                try {
                    Thread.sleep(hypervision.settings().slowPathTimeDelayMS.value);
