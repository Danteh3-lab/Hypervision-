...
public class MovementAscend extends Movement {
    ...
    public MovementAscend(IBaritone hypervision, BetterBlockPos src, BetterBlockPos dest) {
        super(hypervision, src, dest, new BetterBlockPos[]{dest, src.above(2), dest.above()}, dest.below());
    }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (!MovementHelper.canWalkOn(ctx, positionToPlace, jumpingOnto)) {
            ...
            if (MovementHelper.attemptToPlaceABlock(state, hypervision, dest.below(), false, true) == PlaceResult.READY_TO_PLACE) {
        ...
        state.setInput(Input.SNEAK, hypervision.settings().allowWalkOnMagmaBlocks.value && jumpingOnto.is(Blocks.MAGMA_BLOCK));
        ...
        if (hypervision.settings().assumeStep.value || ctx.playerFeet().equals(src.above())) {
