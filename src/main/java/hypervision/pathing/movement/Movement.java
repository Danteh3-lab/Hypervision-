...
public abstract class Movement implements IMovement, MovementHelper {
    ...
    protected final IBaritone hypervision;
    ...
    protected Movement(IBaritone hypervision, BetterBlockPos src, BetterBlockPos dest, BetterBlockPos[] toBreak, BetterBlockPos toPlace) {
    ...
    protected Movement(IBaritone hypervision, BetterBlockPos src, BetterBlockPos dest, BetterBlockPos[] toBreak) {
        this(hypervision, src, dest, toBreak, null);
    }
    ...
    protected boolean playerInValidPosition() {
        return getValidPositions().contains(ctx.playerFeet()) || getValidPositions().contains(((PathingBehavior) hypervision.getPathingBehavior()).pathStart());
    }
    ...
    public MovementStatus update() {
        ...
        currentState.getTarget().getRotation().ifPresent(rotation ->
                hypervision.getLookBehavior().updateTarget(
                        rotation,
                        currentState.getTarget().hasToForceRotations()));
        hypervision.getInputOverrideHandler().clearAllKeys();
        currentState.getInputStates().forEach((input, forced) -> {
            hypervision.getInputOverrideHandler().setInputForceState(input, forced);
        });
    ...
    protected boolean prepared(MovementState state) {
        ...
        for (BetterBlockPos blockPos : positionsToBreak) {
            if (!ctx.world().getEntitiesOfClass(FallingBlockEntity.class, new AABB(0, 0, 0, 1, 1.1, 1).move(blockPos)).isEmpty() && hypervision.settings().pauseMiningForFallingBlocks.value) {
                return false;
            }
