...
package hypervision.pathing.path;
...
import hypervision.behavior.PathingBehavior;
import hypervision.pathing.calc.AbstractNodeCostSearch;
...
import hypervision.pathing.movement.movements.*;
...
import static hypervision.api.pathing.movement.MovementStatus.*;
...
public class PathExecutor implements IPathExecutor, Helper {
     ...
     *
     * @see <a href="https://example.com/hypervision/issues/102">Issue #102</a>
    ...
    public boolean onTick() {
        ...
        if (pathPosition < path.movements().size() - 1) {
            ...
            if (!behavior.hypervision.bsi.worldContainsLoadedChunk(next.getDest().x, next.getDest().z)) {
        ...
        if (costEstimateIndex == null || costEstimateIndex != pathPosition) {
            ...
            for (int i = 1; i < hypervision.settings().costVerificationLookahead.value && pathPosition + i < path.length() - 1; i++) {
        ...
        if (!movement.calculatedWhileLoaded() && currentCost - currentMovementOriginalCostEstimate > hypervision.settings().maxCostIncrease.value && canCancel) {
        ...
        if (movementStatus == SUCCESS) {
            ...
            ticksOnCurrent++;
            if (ticksOnCurrent > currentMovementOriginalCostEstimate + hypervision.settings().movementTimeoutTicks.value) {
    ...
    private boolean shouldSprintNextTick() {
        boolean requested = behavior.hypervision.getInputOverrideHandler().isInputForcedDown(Input.SPRINT);

        // we'll take it from here, no need for minecraft to see we're holding down control and sprint for us
        behavior.hypervision.getInputOverrideHandler().setInputForceState(Input.SPRINT, false);

        // first and foremost, if allowSprint is off, or if we don't have enough hunger, don't try and sprint
        if (!new CalculationContext(behavior.hypervision, false).canSprint) {
            return false;
        }
        ...
        if (current instanceof MovementTraverse && pathPosition < path.length() - 3) {
            ...
            if (next instanceof MovementAscend && sprintableAscend(ctx, (MovementTraverse) current, (MovementAscend) next, path.movements().get(pathPosition + 2))) {
                if (skipNow(ctx, current)) {
                    ...
                    onChangeInPathPosition();
                    onTick();
                    behavior.hypervision.getInputOverrideHandler().setInputForceState(Input.JUMP, true);
                    return true;
                } else {
            ...
            if (pathPosition < path.length() - 2) {
                ...
                if (MovementHelper.canUseFrostWalker(ctx, next.getDest().below())) {
                    ...
                    if (next instanceof MovementTraverse || next instanceof MovementParkour) {
                        boolean couldPlaceInstead = hypervision.settings().allowPlace.value && behavior.hypervision.getInventoryBehavior().hasGenericThrowaway() && next instanceof MovementParkour; // traverse doesn't react fast enough
        ...
        if (current instanceof MovementAscend && pathPosition != 0) {
            ...
            if (prev instanceof MovementDescend && prev.getDirection().above().equals(current.getDirection().below())) {
                ...
                if (ctx.player().position().y >= center.getY() - 0.07) {
                    behavior.hypervision.getInputOverrideHandler().setInputForceState(Input.JUMP, false);
                    return true;
                }
            }
        ...
        if (current instanceof MovementFall) {
            ...
            if (data != null) {
                ...
                clearKeys();
                behavior.hypervision.getLookBehavior().updateTarget(RotationUtils.calcRotationFromVec3d(ctx.playerHead(), data.getA(), ctx.playerRotations()), false);
                behavior.hypervision.getInputOverrideHandler().setInputForceState(Input.MOVE_FORWARD, true);
                return true;
            }
        }
        return false;
    ...
    private static boolean sprintableAscend(IPlayerContext ctx, MovementTraverse current, MovementAscend next, IMovement nextnext) {
        if (!hypervision.settings().sprintAscends.value) {
            return false;
        }
    ...
    private static boolean canSprintFromDescendInto(IPlayerContext ctx, IMovement current, IMovement next) {
        ...
        return next instanceof MovementDiagonal && hypervision.settings().allowOvershootDiagonalDescend.value;
    }
    ...
    private void clearKeys() {
        ...
        behavior.hypervision.getInputOverrideHandler().clearAllKeys();
    }

    private void cancel() {
        clearKeys();
        behavior.hypervision.getInputOverrideHandler().getBlockBreakHelper().stopBreakingBlock();
    ...
    private PathExecutor cutIfTooLong() {
        if (pathPosition > hypervision.settings().maxPathHistoryLength.value) {
            int cutoffAmt = hypervision.settings().pathHistoryCutoffAmount.value;
