...
package hypervision.process.elytra;
...
import hypervision.process.ElytraProcess;
import hypervision.utils.BlockStateInterface;
import hypervision.utils.IRenderer;
import hypervision.utils.PathRenderer;
import hypervision.utils.accessor.IFireworkRocketEntity;
...
import static hypervision.utils.BaritoneMath.fastCeil;
import static hypervision.utils.BaritoneMath.fastFloor;
...
public final class ElytraBehavior implements Helper {
    ...
    public ElytraBehavior(Hypervision hypervision, ElytraProcess process, BlockPos destination, boolean appendDestination) {
        ...
        this.context = new NetherPathfinderContext(hypervision.settings().elytraNetherSeed.value);
    ...
    public void onRenderPass(RenderEvent event) {
        ...
        if (this.visiblePath != null) {
            PathRenderer.drawPath(event.getModelViewStack(), this.visiblePath, 0, new Color(0xC62828), false, 0, 0, 0.0D);
        }
        if (this.aimPos != null) {
            PathRenderer.drawGoal(event.getModelViewStack(), ctx, new GoalBlock(this.aimPos), event.getPartialTicks(), new Color(0xC62828));
        }
        if (!this.clearLines.isEmpty() && settings.elytraRenderRaytraces.value) {
            IRenderer.startLines(new Color(0xD1D5DB), settings.pathRenderLineWidthPixels.value, settings.renderPathIgnoreDepth.value);
            for (Pair<Vec3, Vec3> line : this.clearLines) {
                IRenderer.emitLine(event.getModelViewStack(), line.first(), line.second());
            }
            IRenderer.endLines(settings.renderPathIgnoreDepth.value);
        }
        if (!this.blockedLines.isEmpty() && hypervision.settings().elytraRenderRaytraces.value) {
            IRenderer.startLines(new Color(0x4B5563), settings.pathRenderLineWidthPixels.value, settings.renderPathIgnoreDepth.value);
            for (Pair<Vec3, Vec3> line : this.blockedLines) {
                IRenderer.emitLine(event.getModelViewStack(), line.first(), line.second());
            }
            IRenderer.endLines(settings.renderPathIgnoreDepth.value);
        }
        if (this.simulationLine != null && hypervision.settings().elytraRenderSimulation.value) {
            IRenderer.startLines(new Color(0x9CA3AF), settings.pathRenderLineWidthPixels.value, settings.renderPathIgnoreDepth.value);
    ...
    public void onReceivePacket(PacketEvent event) {
        if (event.getPacket() instanceof ClientboundPlayerPositionPacket) {
            ctx.minecraft().execute(() -> {
                this.remainingSetBackTicks = hypervision.settings().elytraFireworkSetbackUseDelay.value;
            });
        }
    }

    public void pathTo() {
        if (!hypervision.settings().elytraAutoJump.value || ctx.player().isFallFlying()) {
    ...
    public void onTick() {
        ...
        if ((now - this.timeLastCacheCull) / 1000 > hypervision.settings().elytraTimeBetweenCacheCullSecs.value) {
            this.context.queueCacheCulling(ctx.player().chunkPosition().x, ctx.player().chunkPosition().z, hypervision.settings().elytraCacheCullDistance.value, this.boi);
    ...
    /**
     * Called by {@link hypervision.process.ElytraProcess#onTick(boolean, boolean)} when the process is in control and the player is flying
     */
    public void tick() {
        ...
        if (inLava) {
            hypervision.getInputOverrideHandler().setInputForceState(Input.JUMP, true);
        }
        ...
        hypervision.getLookBehavior().updateTarget(solution.rotation, false);
        ...
        for (int relaxation = 0; relaxation < 3; relaxation++) { // try for a strict solution first, then relax more and more (if we're in a corner or near some blocks, it will have to relax its constraints a bit)
            ...
            for (int i = Math.min(playerNear + 20, path.size() - 1); i >= minStep; i--) {
                ...
                for (final Pair<Vec3, Integer> candidate : candidates) {
                    ...
                    final double minAvoidance = hypervision.settings().elytraMinimumAvoidance.value;
    ...
    private void tickUseFireworks(final Vec3 start, final Vec3 goingTo, final boolean isBoosted, final boolean forceUseFirework) {
        ...
        final boolean useOnDescend = !hypervision.settings().elytraConserveFireworks.value || ctx.player().position().y < goingTo.y + 5;
        ...
        final double elytraFireworkSpeed = hypervision.settings().elytraFireworkSpeed.value;
        if (this.remainingFireworkTicks <= 0 && (forceUseFirework || (!isBoosted
            ...
            if (!hypervision.getInventoryBehavior().throwaway(true, ElytraBehavior::isBoostingFireworks) &&
                    !hypervision.getInventoryBehavior().throwaway(true, ElytraBehavior::isFireworks)) {
    ...
    private final class SolverContext {
        ...
        public SolverContext(boolean async) {
            ...
            ITickableAimProcessor aim = ElytraBehavior.this.hypervision.getLookBehavior().getAimProcessor().fork();
            if (async) {
    ...
    private boolean isHitboxClear(final SolverContext context, final Vec3 dest, final Double growAmount) {
        ...
        if (hypervision.settings().elytraRenderHitboxRaytraces.value) {
            boolean clear = true;
            for (int i = 0; i < 8; i++) {
    ...
    public boolean clearView(Vec3 start, Vec3 dest, boolean ignoreLava) {
        ...
        if (hypervision.settings().elytraRenderRaytraces.value) {
    ...
    private static FloatArrayList pitchesToSolveFor(final float goodPitch, final boolean desperate) {
        final float minPitch = desperate ? -90 : Math.max(goodPitch - hypervision.settings().elytraPitchRange.value, -89);
        final float maxPitch = desperate ? 90 : Math.min(goodPitch + hypervision.settings().elytraPitchRange.value, 89);
    ...
    private Pair<Float, Boolean> solvePitch(final SolverContext context, final Vec3 goal, final int relaxation) {
        ...
        final int ticks = desperate ? 3 : context.boost.isBoosted() ? Math.max(5, context.boost.getGuaranteedBoostTicks()) : hypervision.settings().elytraSimulationTicks.value;
    ...
    private void tickInventoryTransactions() {
        if (invTickCountdown <= 0) {
            ...
            if (r != null) {
                r.run();
                invTickCountdown = hypervision.settings().ticksBetweenInventoryMoves.value;
            }
        }
    ...
    private int findGoodElytra() {
        ...
        for (int i = 0; i < invy.size(); i++) {
            ...
            if (slot.getItem() == Items.ELYTRA && (slot.getItem().getMaxDamage() - slot.getDamageValue()) > hypervision.settings().elytraMinimumDurability.value) {
                return i;
            }
        }
        return -1;
    ...
    private void trySwapElytra() {
        if (!hypervision.settings().elytraAutoSwap.value || !invTransactionQueue.isEmpty()) {
            return;
        }

        ItemStack chest = ctx.player().getItemBySlot(EquipmentSlot.CHEST);
        if (chest.getItem() != Items.ELYTRA
                || chest.getItem().getMaxDamage() - chest.getDamageValue() > hypervision.settings().elytraMinimumDurability.value) {
            return;
        }
    ...
    void logVerbose(String message) {
        if (hypervision.settings().elytraChatSpam.value) {
            logDebug(message);
        }
    }
}
