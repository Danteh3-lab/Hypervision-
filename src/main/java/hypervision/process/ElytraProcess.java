...
import hypervision.api.pathing.movement.IMovement;
import hypervision.api.pathing.path.IPathExecutor;
...
import hypervision.api.utils.RotationUtils;
import hypervision.api.utils.input.Input;
import hypervision.pathing.movement.CalculationContext;
import hypervision.pathing.movement.movements.MovementFall;
import hypervision.process.elytra.ElytraBehavior;
import hypervision.process.elytra.NetherPathfinderContext;
import hypervision.process.elytra.NullElytraProcess;
...
public class ElytraProcess extends BaritoneProcessHelper implements IBaritoneProcess, IElytraProcess, AbstractGameEventListener {
    ...
    private ElytraProcess(Hypervision hypervision) {
        super(hypervision);
        hypervision.getGameEventHandler().registerEventListener(this);
    }

    public static IElytraProcess create(final Hypervision hypervision) {
        return NetherPathfinderContext.isSupported()
                ? new ElytraProcess(hypervision)
                : new NullElytraProcess(hypervision);
    }
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        final long seedSetting = hypervision.settings().elytraNetherSeed.value;
        if (seedSetting != this.behavior.context.getSeed()) {
            logDirect("Nether seed changed, recalculating path");
            this.resetState();
        }
        if (predictingTerrain != hypervision.settings().elytraPredictTerrain.value) {
            logDirect("elytraPredictTerrain setting changed, recalculating path");
            predictingTerrain = hypervision.settings().elytraPredictTerrain.value;
            this.resetState();
        }
        ...
        if (ctx.player().isFallFlying() && shouldLandForSafety()) {
            if (hypervision.settings().elytraAllowEmergencyLand.value) {
        ...
        if (ctx.player().isFallFlying() && this.state != State.LANDING && (this.behavior.pathManager.isComplete() || safetyLanding)) {
            ...
            if (last != null && ctx.player().position().distanceToSqr(last.getCenter()) < 1) {
                if (hypervision.settings().notificationOnPathComplete.value && !reachedGoal) {
                    logNotification("Pathing complete", false);
                }
                if (hypervision.settings().disconnectOnArrival.value && !reachedGoal) {
        ...
        if (this.state == State.LANDING) {
            ...
            if (ctx.player().isFallFlying() && endPos != null) {
                ...
                hypervision.getLookBehavior().updateTarget(new Rotation(rotation.getYaw(), 0), false); // this will be overwritten, probably, by behavior tick
        ...
        if (this.state == State.FLYING || this.state == State.START_FLYING) {
            this.state = ctx.player().isOnGround() && hypervision.settings().elytraAutoJump.value
                    ? State.LOCATE_JUMP
                    : State.START_FLYING;
        }
        ...
        if (this.state == State.LOCATE_JUMP) {
            ...
            final IPathExecutor executor = hypervision.getPathingBehavior().getCurrent();
            ...
            return new PathingCommandContext(this.goal, PathingCommandType.SET_GOAL_AND_PAUSE, new WalkOffCalculationContext(hypervision));
        }
        ...
        if (this.state == State.START_FLYING) {
            if (!isSafeToCancel) {
                // owned
                hypervision.getPathingBehavior().secretInternalSegmentCancel();
            }
            hypervision.getInputOverrideHandler().clearAllKeys();
    ...
    private void destroyBehaviorAsync() {
        ...
        if (behavior != null) {
            this.behavior = null;
            hypervision.getExecutor().execute(behavior::destroy);
        }
    }
    ...
    private void pathTo0(BlockPos destination, boolean appendDestination) {
        ...
        this.onLostControl();
        this.predictingTerrain = hypervision.settings().elytraPredictTerrain.value;
        this.behavior = new ElytraBehavior(this.hypervision, this, destination, appendDestination);
        if (ctx.world() != null) {
            this.behavior.repackChunks();
        }
    ...
    private boolean shouldLandForSafety() {
        ...
        if (chest.getItem() != Items.ELYTRA || chest.getItem().getMaxDamage() - chest.getDamageValue() < hypervision.settings().elytraMinimumDurability.value) {
        ...
        if (qty <= hypervision.settings().elytraMinFireworksBeforeLanding.value) {
            return true;
        }
        return false;
    }
    ...
    public void onPostTick(TickEvent event) {
        IBaritoneProcess procThisTick = hypervision.getPathingControlManager().mostRecentInControl().orElse(null);
    ...
    public static final class WalkOffCalculationContext extends CalculationContext {
        ...
        public WalkOffCalculationContext(IBaritone hypervision) {
            super(hypervision, true);
    ...
    private boolean isSafeBlock(Block block) {
        return block == Blocks.NETHERRACK || block == Blocks.GRAVEL || (block == Blocks.NETHER_BRICKS && hypervision.settings().elytraAllowLandOnNetherFortress.value);
    }
