...
import hypervision.api.process.IGetToBlockProcess;
...
public final class GetToBlockProcess extends BaritoneProcessHelper implements IGetToBlockProcess {
    ...
    public GetToBlockProcess(Hypervision hypervision) {
        super(hypervision);
    }
    ...
    public synchronized PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (knownLocations.isEmpty()) {
            if (hypervision.settings().exploreForBlocks.value && !calcFailed) {
        ...
        if (calcFailed) {
            if (hypervision.settings().blacklistClosestOnFailure.value) {
        ...
        if (mineGoalUpdateInterval != 0 && tickCount++ % mineGoalUpdateInterval == 0) { // big brain
            ...
            hypervision.getExecutor().execute(() -> rescan(current, context));
        }
        if (goal.isInGoal(ctx.playerFeet()) && goal.isInGoal(hypervision.getPathingBehavior().pathStart()) && isSafeToCancel) {
            // we're there
    ...
    public class GetToBlockCalculationContext extends CalculationContext {
        ...
        public GetToBlockCalculationContext(boolean forUseOnAnotherThread) {
            super(GetToBlockProcess.super.hypervision, forUseOnAnotherThread);
        }
    ...
    private Goal createGoal(BlockPos pos) {
        ...
        if (blockOnTopMustBeRemoved(gettingTo.getBlock()) && MovementHelper.isBlockNormalCube(hypervision.bsi.get0(pos.above()))) { // TODO this should be the check for chest openability
    ...
    private boolean rightClick() {
        for (BlockPos pos : knownLocations) {
            ...
            if (reachable.isPresent()) {
                hypervision.getLookBehavior().updateTarget(reachable.get(), true);
                if (knownLocations.contains(ctx.getSelectedBlock().orElse(null))) {
                    hypervision.getInputOverrideHandler().setInputForceState(Input.CLICK_RIGHT, true); // TODO find some way to right click even if we're in an ESC menu
    ...
    private boolean walkIntoInsteadOfAdjacent(Block block) {
        if (!hypervision.settings().enterPortal.value) {
            return false;
        }
    ...
    private boolean rightClickOnArrival(Block block) {
        if (!hypervision.settings().rightClickContainerOnArrival.value) {
            return false;
        }
