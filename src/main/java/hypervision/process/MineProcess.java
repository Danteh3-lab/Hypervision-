...
import hypervision.api.pathing.goals.*;
import hypervision.api.process.IMineProcess;
...
public final class MineProcess extends BaritoneProcessHelper implements IMineProcess {
    ...
    public MineProcess(Hypervision hypervision) {
        super(hypervision);
    }
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        if (calcFailed) {
            if (!knownOreLocations.isEmpty() && hypervision.settings().blacklistClosestOnFailure.value) {
                logDirect("Unable to find any path to " + filter + ", blacklisting presumably unreachable closest instance...");
                if (hypervision.settings().notificationOnMineFail.value) {
        ...
        updateLoucaSystem();
        int mineGoalUpdateInterval = hypervision.settings().mineGoalUpdateInterval.value;
        List<BlockPos> curr = new ArrayList<>(knownOreLocations);
        if (mineGoalUpdateInterval != 0 && tickCount++ % mineGoalUpdateInterval == 0) { // big brain
            CalculationContext context = new CalculationContext(hypervision, true);
            hypervision.getExecutor().execute(() -> rescan(curr, context));
        }
        if (hypervision.settings().legitMine.value) {
            if (!addNearby()) {
                cancel();
                return null;
            }
        ...
        if (shaft.isPresent() && ctx.player().isOnGround()) {
            ...
            BlockState state = hypervision.bsi.get0(pos);
            if (!MovementHelper.avoidBreaking(hypervision.bsi, pos.getX(), pos.getY(), pos.getZ(), state)) {
                Optional<Rotation> rot = RotationUtils.reachable(ctx, pos);
                if (rot.isPresent() && isSafeToCancel) {
                    hypervision.getLookBehavior().updateTarget(rot.get(), true);
    ...
    private void updateLoucaSystem() {
        ...
        ctx.getSelectedBlock().ifPresent(pos -> {
            if (knownOreLocations.contains(pos)) {
                copy.put(pos, System.currentTimeMillis() + hypervision.settings().mineDropLoiterDurationMSThanksLouca.value);
            }
        });
    ...
    private PathingCommand updateGoal() {
        ...
        boolean legit = hypervision.settings().legitMine.value;
        List<BlockPos> locs = knownOreLocations;
        if (!locs.isEmpty()) {
            CalculationContext context = new CalculationContext(hypervision);
            List<BlockPos> locs2 = prune(context, new ArrayList<>(locs), filter, hypervision.settings().mineMaxOreLocationsCount.value, blacklist, droppedItemsScan());
        ...
        if (!legit && !hypervision.settings().exploreForBlocks.value) {
            return null;
        }
        // only when we should explore for blocks or are in legit mode we do this
        int y = hypervision.settings().legitMineYLevel.value;
        if (branchPoint == null) {
            /*if (!hypervision.getPathingBehavior().isPathing() && playerFeet().y == y) {
    ...
    private void rescan(List<BlockPos> already, CalculationContext context) {
        ...
        List<BlockPos> locs = searchWorld(context, filter, hypervision.settings().mineMaxOreLocationsCount.value, already, blacklist, dropped);
        locs.addAll(dropped);
        if (locs.isEmpty() && !hypervision.settings().exploreForBlocks.value) {
    ...
    private boolean internalMiningGoal(BlockPos pos, CalculationContext context, List<BlockPos> locs) {
        ...
        if (hypervision.settings().internalMiningAirException.value && state.getBlock() instanceof AirBlock) {
            return true;
        }
    ...
    private Goal coalesce(BlockPos loc, List<BlockPos> locs, CalculationContext context) {
        boolean assumeVerticalShaftMine = !(hypervision.bsi.get0(loc.above()).getBlock() instanceof FallingBlock);
        if (!hypervision.settings().forceInternalMining.value) {
    ...
    public List<BlockPos> droppedItemsScan() {
        if (!hypervision.settings().mineScanDroppedItems.value) {
    ...
    public static List<BlockPos> searchWorld(CalculationContext ctx, BlockOptionalMetaLookup filter, int max, List<BlockPos> alreadyKnown, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        for (BlockOptionalMeta bom : filter.blocks()) {
            ...
            if (CachedChunk.BLOCKS_TO_KEEP_TRACK_OF.contains(block)) {
                BetterBlockPos pf = ctx.hypervision.getPlayerContext().playerFeet();

                // maxRegionDistanceSq 2 means adjacent directly or adjacent diagonally; nothing further than that
                locs.addAll(ctx.worldData.getCachedWorld().getLocationsOf(
                        BlockUtils.blockToString(block),
                        hypervision.settings().maxCachedWorldScanCount.value,
                        pf.x,
                        pf.z,
                        2
                ));
        ...
        if (!untracked.isEmpty() || (hypervision.settings().extendCacheOnThreshold.value && locs.size() < max)) {
            locs.addAll(HypervisionAPI.getProvider().getWorldScanner().scanChunkRadius(
    ...
    private boolean addNearby() {
            ...
            for (int y = playerFeet.getY() - searchDist; y <= playerFeet.getY() + searchDist; y++) {
                for (int z = playerFeet.getZ() - searchDist; z <= playerFeet.getZ() + searchDist; z++) {
                    ...
                    if (filter.has(bsi.get0(x, y, z))) {
                        ...
                        if ((hypervision.settings().legitMineIncludeDiagonals.value && knownOreLocations.stream().anyMatch(ore -> ore.distSqr(pos) <= 2 /* sq means this is pytha dist <= sqrt(2) */)) || RotationUtils.reachable(ctx, pos, fakedBlockReachDistance).isPresent()) {
        ...
        knownOreLocations = prune(new CalculationContext(hypervision), knownOreLocations, filter, hypervision.settings().mineMaxOreLocationsCount.value, blacklist, dropped);
        return true;
    }
    ...
    private static List<BlockPos> prune(CalculationContext ctx, List<BlockPos> locs2, BlockOptionalMetaLookup filter, int max, List<BlockPos> blacklist, List<BlockPos> dropped) {
        ...
        List<BlockPos> locs = locs2
                ...
                .filter(pos -> {
                    if (hypervision.settings().allowOnlyExposedOres.value) {
                ...
                .filter(pos -> pos.getY() >= hypervision.settings().minYLevelWhileMining.value + ctx.world.dimensionType().minY())

                .filter(pos -> pos.getY() <= hypervision.settings().maxYLevelWhileMining.value)
    ...
    public static boolean isNextToAir(CalculationContext ctx, BlockPos pos) {
        int radius = hypervision.settings().allowOnlyExposedOresDistance.value;
    ...
    public void mine(int quantity, BlockOptionalMetaLookup filter) {
        ...
        if (filter != null) {
            rescan(new ArrayList<>(), new CalculationContext(hypervision));
        }
    }
    ...
    private BlockOptionalMetaLookup filterFilter() {
        ...
        if (!hypervision.settings().allowBreak.value) {
            BlockOptionalMetaLookup f = new BlockOptionalMetaLookup(this.filter.blocks()
                    .stream()
                    .filter(e -> hypervision.settings().allowBreakAnyway.value.contains(e.getBlock()))
