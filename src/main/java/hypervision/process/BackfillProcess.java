...
public final class BackfillProcess extends BaritoneProcessHelper {
    ...
    public BackfillProcess(Hypervision hypervision) {
        super(hypervision);
    }
    ...
    public boolean isActive() {
        ...
        if (!hypervision.settings().backfill.value) {
            return false;
        }
        if (hypervision.settings().allowParkour.value) {
            logDirect("Backfill cannot be used with allowParkour true");
            hypervision.settings().backfill.value = false;
            return false;
        }
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        ...
        for (BlockPos toPlace : toFillIn()) {
            ...
            switch (MovementHelper.attemptToPlaceABlock(fake, hypervision, toPlace, false, false)) {
                case NO_OPTION:
                    continue;
                case READY_TO_PLACE:
                ...
                case ATTEMPTING:
                    // patience
                    hypervision.getLookBehavior().updateTarget(fake.getTarget().getRotation().get(), true);
                    return new PathingCommand(null, PathingCommandType.REQUEST_PAUSE);
                default:
    ...
    private void amIBreakingABlockHMMMMMMM() {
        if (!ctx.getSelectedBlock().isPresent() || !hypervision.getPathingBehavior().isPathing()) {
            return;
        }
    ...
    public List<BlockPos> toFillIn() {
        return blocksToReplace
                ...
                .filter(pos -> hypervision.getBuilderProcess().placementPlausible(pos, Blocks.DIRT.defaultBlockState()))
    ...
    private boolean partOfCurrentMovement(BlockPos pos) {
        PathExecutor exec = hypervision.getPathingBehavior().getCurrent();
