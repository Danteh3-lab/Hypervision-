...
public final class CustomGoalProcess extends BaritoneProcessHelper implements ICustomGoalProcess {
    ...
    public CustomGoalProcess(Hypervision hypervision) {
        super(hypervision);
    }
    ...
    public void setGoal(Goal goal) {
        ...
        if (hypervision.getElytraProcess().isActive()) {
            hypervision.getElytraProcess().pathTo(goal);
        }
    ...
    public PathingCommand onTick(boolean calcFailed, boolean isSafeToCancel) {
        switch (this.state) {
            ...
            case EXECUTING:
                ...
                if (this.goal == null || (this.goal.isInGoal(ctx.playerFeet()) && this.goal.isInGoal(hypervision.getPathingBehavior().pathStart()))) {
                    onLostControl(); // we're there xd
                    if (hypervision.settings().disconnectOnArrival.value) {
                        ctx.world().disconnect();
                    }
                    if (hypervision.settings().notificationOnPathComplete.value) {
