...
import hypervision.api.pathing.goals.GoalNear;
import hypervision.api.pathing.goals.GoalXZ;
import hypervision.api.process.IFollowProcess;
...
public final class FollowProcess extends BaritoneProcessHelper implements IFollowProcess {
    ...
    public FollowProcess(Hypervision hypervision) {
        super(hypervision);
    }
    ...
    private Goal towards(Entity following) {
        BlockPos pos;
        if (hypervision.settings().followOffsetDistance.value == 0 || into) {
            pos = following.blockPosition();
        } else {
            GoalXZ g = GoalXZ.fromDirection(following.position(), hypervision.settings().followOffsetDirection.value, hypervision.settings().followOffsetDistance.value);
            pos = new BetterBlockPos(g.getX(), following.position().y, g.getZ());
        }
        if (into) {
            return new GoalBlock(pos);
        }
        return new GoalNear(pos, hypervision.settings().followRadius.value);
    }
    ...
    private boolean followable(Entity entity) {
        ...
        int maxDist = hypervision.settings().followTargetMaxDistance.value;
